...
@Component({
  ...
  styleUrls: ['./sensores-list.component.scss']
})
export class SensoresListComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    this.route.params.subscribe(async (params: any) => {
      ...
      if (this.items[0]?.id) {
        ...
        let empresa = this.id;
        this.getRealtimeSensors(empresa);
  ...
  getRealtimeSensors(empresa: any) {
    empresa = empresa.toString();
    const realtimeData = this.firebaseService.getRef(`${empresa}/sensores`);
  ...
  async getData(id: number) {
    let response = await this.service.getAllByEmpresa(id);
